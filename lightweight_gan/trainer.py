...
from lightweight_gan.lightweight_gan import LightweightGAN
from lightweight_gan.loss_fns import dual_contrastive_loss, gen_hinge_loss, hinge_loss
...
class Trainer:
        ...
        **kwargs,
        ...
        if self.is_main and use_aim:
            try:
                ...
                print(
                    "unable to import aim experiment tracker - please run `pip install aim` first"
                )
    ...
    def evaluate(self, num=0, num_image_tiles=4):
        ...
        interpolate_latents = interpolate_between(
            latents[:num_rows], latents[-num_rows:], num_samples=num_rows, dim=0
        ).flatten(end_dim=1)

        generate_interpolations = self.generate_(self.GAN.G, interpolate_latents)
        if self.run is not None:
            grouped = generate_interpolations.view(
                num_rows, num_rows, *generate_interpolations.shape[1:]
            )
            for idx, images in enumerate(grouped):
                ...
                self.run.track(
                    ...
                    name="generated",
                    step=self.steps,
                    context={"interpolated": True, "alpha": alpha},
                )
